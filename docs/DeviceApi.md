...
[**set_device_token**](DeviceApi.md#set_device_token) | **PUT** /device/token | 
[**set_status**](DeviceApi.md#set_status) | **PUT** /device/status | 
...
# **set_device_token**
> set_device_token(device_token_dto)
...
with thystreet.ApiClient(configuration) as api_client:
    ...
    # example passing only required values which don't have defaults set
    try:
        api_instance.set_device_token(device_token_dto)
    except thystreet.ApiException as e:
        print("Exception when calling DeviceApi->set_device_token: %s\n" % e)
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **device_status_dto** | [**DeviceStatusDto**](DeviceStatusDto.md)|  |
